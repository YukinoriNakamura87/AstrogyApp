...
def _essential(planet: PlanetInput, planets: dict[P, PlanetInput], sect: str) -> ScoreSection:
    ...
    if term_ruler == planet.name:
        items.append(_item("term", "ターム", 2, f"{sign} {degree:.6f}°のターム支配"))
        positive_dignity = True
    face_ruler = FACES[sign][min(int(degree // 10), 2)]
    if face_ruler == planet.name:
        items.append(_item("face", "フェイス", 1, f"{sign} {degree:.6f}°のフェイス支配"))
        positive_dignity = True
...
def _aspect_item(target: PlanetInput, other: PlanetInput, rules: tuple[tuple[float, int, str], ...], prefix: str) -> ScoreItem | None:
    ...
    for angle, points, label in rules:
        if abs(separation - angle) <= 1:
            return _item(f"{prefix}_{int(angle)}", label, points, f"{PLANET_LABELS[other.name]}とパータイル（オーブ {abs(separation-angle):.6f}°）", other.name.value)
    return None
...
def _accidental(planet: PlanetInput, planets: dict[P, PlanetInput], chart: ChartInput) -> ScoreSection:
    ...
    items.append(_item("house", f"第{house}ハウス", HOUSE_POINTS[house], f"在室ハウス配点{'（5度前ルール適用、カスプまで '+format(cusp_distance, '.6f')+'°）' if moved else ''}"))
    ...
    if isclose(motion, average, rel_tol=0, abs_tol=1e-9):
        items.append(_item("average_speed", "平均速度と同じ", 0, f"日運動 {motion:.6f}°", status="neutral"))
    elif motion > average:
        items.append(_item("fast", "平均より速い", 2, f"日運動 {motion:.6f}° > 平均 {average:.6f}°"))
    else:
        items.append(_item("slow", "平均より遅い", -2, f"日運動 {motion:.6f}° < 平均 {average:.6f}°"))
    ...
    if planet.name != P.SUN:
        ...
        if separation > 1:
            ...
            items.append(_item("oriental" if oriental else "occidental", label, points, f"太陽から天体への黄経差 {planet_from_sun:.6f}°"))

        same_sign = _sign(planet.longitude) == _sign(sun.longitude)
        if separation <= 17 / 60:
            items.append(_item("cazimi", "カジミ", 5, f"太陽との離角 {separation:.6f}°"))
        elif separation <= 8.5 and same_sign:
            items.append(_item("combust", "コンバスト", -5, f"太陽との離角 {separation:.6f}°、同一サイン"))
        elif separation <= 17:
            items.append(_item("under_sunbeams", "アンダー・ザ・サンビーム", -4, f"太陽との離角 {separation:.6f}°"))
        else:
            items.append(_item("free_from_sunbeams", "太陽光線外", 5, f"太陽との離角 {separation:.6f}°"))
    ...
    for longitude, code, label, points in ((chart.north_node_longitude, "north_node_conjunction", "ドラゴンヘッド合", 4), (chart.south_node_longitude, "south_node_conjunction", "ドラゴンテイル合", -4)):
        if longitude is not None and _distance(planet.longitude, longitude) <= 1:
            items.append(_item(code, label, points, f"コンジャンクション、オーブ {_distance(planet.longitude, longitude):.6f}°"))
    ...
    if planet.name not in {P.MARS, P.SATURN} and _distance(mars.longitude, saturn.longitude) <= 6 and _between_on_short_arc(planet.longitude, mars.longitude, saturn.longitude):
        items.append(_item("besieged", "火星・土星に挟まれる", -5, f"火星と土星の最短角距離 {_distance(mars.longitude, saturn.longitude):.6f}°"))
    ...
    if missing_stars:
        ...
        for name, (points, orb, label) in STAR_RULES.items():
            ...
            if distance <= orb:
                items.append(_item(f"fixed_star_{name.lower()}", f"{label}とのコンジャンクション", points, f"オーブ {distance:.6f}°", name))
    return ScoreSection(sum(item.points for item in items), tuple(items))
